...
public class Threshold {
    ...
    public static void doubleThreshold(BufferedImage NMSimage,BufferedImage gradImage,String imageSaveDir){
        ...
        for(int x=xStart;x<width;x++){
            for(int y=yStart;y<height;y++){
                if(ImageBaseOp.getRgbArrary(NMSimage,x,y)[0] > threshold[1]){
                    hightTimage.setRGB(x,y,0xffffffff);
                    outImage.setRGB(x,y,0xffffffff);
                }else{
                    hightTimage.setRGB(x,y,0x00000000);
                }
                if (ImageBaseOp.getRgbArrary(NMSimage,x,y)[0] > threshold[0]){
        ...
        ImageBaseOp.SaveImage(outImage,"jpg",new File(imageSaveDir+"result.jpg"));
        ImageBaseOp.SaveImage(hightTimage,"jpg",new File(imageSaveDir+"hight.jpg"));
        ImageBaseOp.SaveImage(lowTimage,"jpg",new File(imageSaveDir+"low.jpg"));
        ImageBaseOp.SaveImage(weakEdge,"jpg",new File(imageSaveDir+"weak.jpg"));
    }
        ...
        for(int x=xStart; x<width; x++) {
            for(int y=yStart; y<height; y++)
            {
                if(NmsImage.getRGB(x,y)!=0)//???????????????????????????
                    hist[ImageBaseOp.getRgbArrary(gradImage,x,y)[0]]++;
            }
        }
        return hist;
    }
